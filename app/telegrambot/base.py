...
class Chat(BaseModel):
    ...
    username: Optional[str] = None # Username, for private chats, supergroups and channels if available
...
class Message(BaseModel):
    ...
    """
    message_id: int
    from_user: Optional[User] = Field(None, alias="from") # Sender of the message; can be empty for messages sent to channels
    chat: Chat # Conversation the message belongs to
    date: int # Date the message was sent in Unix time
    text: Optional[str] = None # For text messages, the actual UTF-8 text of the message
...
class TelegramUpdate(BaseModel):
    ...
    message: Optional[Message] = None # New incoming message of any kind — text, photo, sticker, etc.
    edited_message: Optional[Message] = None # New version of a message that is known to the bot and was edited
    channel_post: Optional[Message] = None # New incoming channel post of any kind — text, photo, sticker, etc.
    edited_channel_post: Optional[Message] = None # New version of a channel post that is known to the bot and was edited
