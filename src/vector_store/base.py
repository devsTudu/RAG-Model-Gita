...
class RobustPGVectorRetriever(BaseRetriever):
    ...
    def __init__(self, **data: Any):
        ...
        if self.search_type == "mmr" and (self.fetch_k is None or self.lambda_mult is None):
            logger.warning("For 'mmr' search_type, 'fetch_k' and 'lambda_mult' are recommended for optimal performance.")
    ...
    def _perform_pgvector_search(self, query: str) -> List[Document]:
        ...
        """
        logger.info(f"Attempting PGVector search with type: {self.search_type} for query: '{query}'")
        try:
            if self.search_type == "similarity":
                ...
                docs = [doc for doc, score in docs_with_scores if score >= self.score_threshold]
            else:
    ...
    def _get_relevant_documents(
        ...
        """
        Get relevant documents from the PGVector database, with robustness measures.
        """
        ...
        try:
            ...
            logger.info(f"Successfully retrieved {len(retrieved_docs)} documents from PGVector.")
            return retrieved_docs
        except LookupError as e:
            logger.error(f"PGVector search failed after multiple retries for query '{query}': {e}")
            if self.enable_fallback and self.fallback_retriever:
                logger.warning(f"Falling back to alternative retriever for query: '{query}'")
                try:
                    fallback_docs = self.fallback_retriever.get_relevant_documents(query, run_manager=run_manager)
                    logger.info(f"Successfully retrieved {len(fallback_docs)} documents from fallback retriever.")
                    return fallback_docs
                except Exception as fe:
                    logger.error(f"Fallback retriever also failed for query '{query}': {fe}")
                    raise RuntimeError(f"Both primary and fallback retrievers failed for query: '{query}'") from fe
            else:
                logger.error(f"No fallback retriever configured or enabled. Raising original error.")
                raise RuntimeError(f"Failed to retrieve documents from PGVector after retries: {e}") from e
        except Exception as e:
            logger.error(f"An unexpected error occurred during document retrieval for query '{query}': {e}")
            raise RuntimeError(f"An unexpected error occurred during document retrieval: {e}") from e
